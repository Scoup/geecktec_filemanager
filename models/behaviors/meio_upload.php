 ...
 */
	function _createThumbnail(&$model, $source, $target, $fieldName, $params = array()) {
		...
		App::import('Vendor', 'GeecktecFilemanager.phpthumb', array('file' => 'phpThumb' . DS . 'phpthumb.class.php'));
 ...
 */
	...
	function _copyFileFromTemp($tmpName, $saveAs) {
		...
		$tmp = explode(DS, $tmpName);
		$tempFile = '';
		$count = count($tmp);
		for($i = 0; $i < $count - 1; $i++) {
			$tempFile .= $tmp[$i].DS;
		}
		if (!is_uploaded_file($tmpName) && $tempFile != $tmpFolder.DS) {
			return false;
		}
		if (!move_uploaded_file($tmpName, $saveAs) && !copy($tmpName, $saveAs)) {
 ...
 */
	function _setFileToRemove(&$model, $fieldName) {
		...
		if (!empty($filename)) {
			...
			/**
			 * changes to geecktec_filemanager
			 * now can create news thumbs in geecktec_filemanager
			 */
			$dir = $this->__fields[$model->alias][$fieldName]['dir'] . DS . 'thumb'; 
